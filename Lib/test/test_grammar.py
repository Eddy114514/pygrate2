...
''')
    ...
    def test_StringIO_py3k_warnings(self):
        if sys.py3kwarning:
            with warnings.catch_warnings(record=True) as w:
                warnings.filterwarnings('always')
                src = "import StringIO"
                ...
                for warning in w:
                    self.assertTrue(Py3xWarning is w.category)
                    self.assertEqual(str(w.message), "Py3xWarning: module 'StringIO' is removed in Python 3:"
                                     " use 'io.StringIO' (text) or 'io.BytesIO' (bytes)")

    def test_cStringIO_py3k_warnings(self):
        if sys.py3kwarning:
            with warnings.catch_warnings(record=True) as w:
                warnings.filterwarnings('always')
                src = "import cStringIO"
                compile(src, '<exec_warning_test>', 'exec')
    
                for warning in w:
                    self.assertTrue(Py3xWarning is w.category)
                    self.assertEqual(str(w.message), "Py3xWarning: module 'cStringIO' is removed in Python 3"
                                     ": use 'io.StringIO' (text) or 'io.BytesIO' (bytes)")
